 ...
 {   
		...
		if (empty($errors) ) 
		{
				...
				if($_POST["cbactive"]=="on")
					$i=1;
				$c_date = date("Y-m-d");
				$str="update kitchencategories set imgcatname='".$_POST["imgcatname"]."',active=b'".$i."',modifyip='".getip()."',modifydate='".$c_date."' where imgcatid=".$_GET["id"];
		//		echo $str;
				$cnt=ExecuteNonQuery($str);
				if($cnt==1)
				{
					redirect_to("viewkitchencategories.php");	
				}
		}
		else
		{
...
<div id="wrapper">        
    <div id="main">
      ...
      <div id="content">
			...
			<form method="post" action="editkitchencategory.php?id=<?php echo $_GET["id"];?>">
            	 <?php 
				$data = ExecuteNonQuery('select * FROM kitchencategories where imgcatid='.$_GET["id"]) or die(mysql_error());
